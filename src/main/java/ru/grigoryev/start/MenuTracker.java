...
public class MenuTracker {
	...
	*/
	public MenuTracker(Input input, Tracker tracker) {
		...
		this.actions = new UserAction[100];
		this.fillActions();
	...
	*/
	private void fillActions() {
		this.actions[this.position++] = new AddItem("Add new item.", this.ADD);
		this.actions[this.position++] = new MenuTracker.ShowAll("Show all items.", this.SHOW_ALL);
		this.actions[this.position++] = new UpdateItem("Update item.", this.EDIT);
		this.actions[this.position++] = new DeleteItem("Delete item.", this.DELETE);
		this.actions[this.position++] = new MenuTracker.FindByID("Find the item by id.", this.FIND_BY_ID);
	};
	/**
	*This method make range array in appropriate state with actions.
	*/
	private void updateRange() {
		this.range = new int[this.position + 1]; // в массиве диапазонов содержится также ключ EXIT в отличие от массива action
		for (int i = 0; i < this.position; i++) {
			this.range[i] = this.actions[i].key(); // Заполняем массив диапазонов значениями ключей каждого действия в меню
		}
		this.range[this.position] = EXIT; // Последнее число в массиве диапазонов представляет выход (EXIT)
	};
	/**
	...
	*/
	public void addAction(UserAction action) {
		if (this.position < actions.length) {
